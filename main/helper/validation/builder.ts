import { ValidationResult , ValidationErrorItem , ValidationError } from 'joi';
...
export class ValidationMessageBuilder {
	...
	public static build(err : ValidationResult , schemaType : string) : ValidationErrorMessage[] {
			...
			for (let i in $details) { let validationErrorItem : ValidationErrorItem = $details[i];
	...
	public static transformValidationErrorsToSArrayOfString(validationErrorMessages : ValidationErrorMessage[]) : string[] {

		let transformErrorList : string[] = validationErrorMessages.map((item : ValidationErrorMessage) => {
