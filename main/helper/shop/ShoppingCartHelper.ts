...
export class ShoppingCartHelper {
	...
	public static getTotalAmount(cart : ShoppingCart) : number {
		...
		cart.items.forEach(function(item : OrderItem) : void {

			totalAmt += ShoppingCartHelper.getItemTotalAmount(item); });
	...
	public static removeItem(cart : ShoppingCart , item_id : number) {
		...
		cart.items = cart.items.filter(function(existingItem : OrderItem) : unknown {
				...
				if (existingItem.item_id !== item_id) {

					return true; }

				else { return false; } });

		if (cart.items.length > 0 && orderItem !== null) { 
	...
	public static getItemTotalAmount(orderItem : OrderItem) : number {
	...
	public static addItem(cart : ShoppingCart , item : OrderItem) : void {
		...
		else if (item_index === cart.items.length && item.quantity !== 0) {

			cart.items[cart.items.length] = item;
		}

		else {

			ShoppingCartHelper.removeItem(cart , item.item_id);
	...
	public static computeItemsData(cart : ShoppingCart) : ShoppingCart | null {
		...
		if (cart !== null) {
			...
			cart.items = cart.items.map(function(item : OrderItem) : OrderItem {

					item.amount = ShoppingCartHelper.getItemTotalAmount(item);
	...
	private static setitem(cart : ShoppingCart , item_index : number , item : OrderItem) : void {

		cart.items[item_index] = item;
	}
	...
	public static updateItem(cart : ShoppingCart , item_id : number , quantity : number) : void {
		...
		if (quantity < 1 && orderItem !== null) {

			ShoppingCartHelper.removeItem(cart , item_id);
	...
	public static setOrderItems(items : OrderItem[] , order : Order) : void {
		...
		items.forEach((item : OrderItem) => {
				...
				let orderItem : OrderItem = new OrderItem(item.item_id , item.title , item.unit_price , item.order_id);
